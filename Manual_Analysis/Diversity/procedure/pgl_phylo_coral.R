...
pgl.output
write.table(pgl.output,file="coral_output/beta_diversity/PGL_coral_output_genomes_beta_diversity.csv",append=TRUE, sep=",", row.names=FALSE, col.names=FALSE)



trait_table_input <-"input/tir_pcoa_merged_trait_table.csv"

#Pick the columns to analyze
#y traits can be: obs_asvs, ln_asvs, dominance, gini_index, simpson_e, faith_pd
y_trait_column <-"skeleton_weighted_unifrac_ordination_PC1"
...
pdf(paste("coral_output/beta_diversity/",x_trait_column,"_",y_trait_column,"_phylomorphospace.pdf",sep=""))
phylomorphospace(tree.prune,trait_table[,c(x_trait_column,y_trait_column)],xlab=x_trait_column,ylab=y_trait_column)
...
pdf(paste("coral_output/beta_diversity/",x_trait_column,"_",y_trait_column,"_pic_scatter_YX_no_galaxea.pdf",sep=""))
plot(y_trait_positive ~ x_trait_positive,xlab=x_trait_column,ylab=y_trait_column,bg='gray',pch=16)
abline(a = 0, b = coef(pic_model))
dev.off()

#save the PIC contrasts with the 95% confidence interval as a pdf
pdf(paste("coral_output/beta_diversity/",x_trait_column,"_",y_trait_column,"_pic_scatter_YX_confidence.pdf"))
ggplot(pic_df, aes(x_trait_positive,y_trait_positive)) +
  geom_smooth(method = "lm", se = TRUE, col = "black", formula = y~x -1)+
  geom_point(size = 3, col = "firebrick")+
  labs(x = paste("Contrast in ", x_trait_column), y = paste("Contrast in ", y_trait_column))+
...
#save plot as pdf
pdf(paste("coral_output/beta_diversity/",x_trait_column,"_contmap.pdf",sep=""))
plot(x_trait_reconstruction,direction="rightwards")
dev.off()
...
#save plot as pdf
pdf(paste("coral_output/beta_diversity/",y_trait_column,"_contmap.pdf",sep=""))
plot(y_trait_reconstruction,direction="leftwards")
dev.off()
...
write.table(output_row, file="coral_output/beta_diversity/PGL_coral_output_genomes_beta_diversity.csv", append=TRUE, sep=",", row.names=FALSE, col.names=FALSE)
...
pgl_input_table <- "coral_output/PGL_coral_output_genomes.csv"
pgl_table <- read.csv(pgl_input_table,header=TRUE)
pgl_table.df <- data.frame(pgl_table)
p_value_columns <- pgl_table.df[,c("Immune_Trait", "Diversity_Trait","pVal_Model_1", "pVal_Model_2", "pVal_Model_3", "pVal_Model_4", "pVal_PIC")]
#p_value_columns
...
Immune_trait_variable <- p_value_columns[grep("ln_asvs", p_value_columns$Immune_Trait),]
#note: The dash needs to be added in front of weighted_unifrac. Otherwise unweighted also gets included.
...
#p value columns can be "p_val_Model_1","p_val_Model_2","p_val_Model_3","p_val_model_4", "pVal_PIC"
p_value_select <- Immune_trait_variable$pVal_PIC
p_value_select
